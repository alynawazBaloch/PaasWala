...
type RouteProps = RouteProp<AppStackParamList, 'FollowingList'>;
...
const FollowingListScreen: React.FC = () => {
  ...
  const { following, unfollow, isFollowing } = useFollow(isOwn ? undefined : userId);
  const [users, setUsers] = useState<FollowingWithUser[]>([]);
  ...
  useEffect(() => {
    ...
    Promise.all(
      following.map(async (conn) => {
        const u = await getUserById(conn.toUserId);
        if (!u) return null;
        return { connectionId: conn.id, user: u };
      })
    ).then((results) => {
      if (cancelled) return;
      setUsers(results.filter(Boolean) as FollowingWithUser[]);
      setLoading(false);
    });
  ...
  const renderItem = ({ item }: { item: FollowingWithUser }) => {
    ...
    return (
      <View style={styles.itemRow}>
        ...
        {isFollowing(item.user.uid) && (
          <TouchableOpacity style={styles.unfollowBtn} onPress={() => handleUnfollow(item.user.uid)}>
...
const styles = StyleSheet.create({
  ...
  unfollowBtnText: { fontSize: 12, fontWeight: '600', color: Colors.error, fontFamily: 'Inter' },
});
